...
def PointInCropToFullSpeed(center_coords, crop_size, full_image_size, crop_keypoints, normalized=True):
    c_x_n, c_y_n = center_coords
    w_n, h_n = crop_size
    W, H = full_image_size
    ...
    c_x = c_x_n * W
    c_y = c_y_n * H
    w = float(w_n * W)  # Ensure this is a float
    h = float(h_n * H)  # Ensure this is a float

    # Calculate the top-left corner of the crop
    x = float(c_x - w / 2)  # Ensure this is a float
    y = float(c_y - h / 2)  # Ensure this is a float

    # Ensure crop_keypoints is a numpy array
    crop_keypoints = np.array(crop_keypoints)

    # Convert normalized coordinates to pixel coordinates
    p_x = crop_keypoints[:, 0] * w  # Element-wise multiplication
    p_y = crop_keypoints[:, 1] * h  # Element-wise multiplication

    # Convert to full image pixel coordinates, broadcast scalar x and y
    P_x = p_x + x
    P_y = p_y + y
    ...
    if normalized:
        ...
        U = P_x / W
        V = P_y / H
        return np.column_stack((U, V))  # Stack columns to return as an array
    else:
